...
def test_multiple_param_build(multiple_param_config):
    ...
    command = step.build_command({})[0]
    assert command.startswith('echo')
    assert all(x in command for x in [
    ...
    command = step.build_command({'encoder-layers': 6, 'denoising-cost-x': (2, 'e')})[0]
    assert command.startswith('echo')
    assert all(x in command for x in [
        '--unlabeled-samples=60000',
    ...
    command = step.build_command({'encoder-layers': None, 'denoising-cost-x': (2, 'e')})[0]
    assert command.startswith('echo')
    assert all(x in command for x in [
        '--unlabeled-samples=60000',
        '--denoising-cost-x=2,e',  # dash-separated
        '--decoder-spec gauss',  # comma-separated
        '--decoder-spec railgun',  # repeated
    ])

    command = step.build_command({'encoder-layers': [], 'denoising-cost-x': (2, 'e')})[0]
    assert command.startswith('echo')
    assert all(x in command for x in [
        '--unlabeled-samples=60000',
        '--denoising-cost-x=2,e',  # dash-separated
...
def test_multiple_param_validate(multiple_param_config):
    ...
    assert step.parameters['denoising-cost-x'].validate(.2) == [.2]

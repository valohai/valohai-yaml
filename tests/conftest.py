from tests.utils import config_fixture

boolean_param_config = config_fixture("flag-param-example.yaml")
boolean_param_pass_as_config = config_fixture("flag-param-pass-as-example.yaml")
edge_merge_mode_config = config_fixture("edge-merge-mode.yaml")
endpoint_config = config_fixture("endpoint-example.yaml")
environment_variable_group_example_config = config_fixture("environment-variable-group-example.yaml")
example1_config = config_fixture("example1.yaml")
example2_config = config_fixture("example2.yaml")
example3_config = config_fixture("example3.yaml")
input_extras_config = config_fixture("input-extras.yaml")
keep_order_config = config_fixture("keep-order.yaml")
mount_config = config_fixture("mount-example.yaml")
multiple_param_config = config_fixture("multiple-param.yaml")
optional_default_param_config = config_fixture("optional-default-param.yaml")
pipeline_config = config_fixture("pipeline-example.yaml")
pipeline_overridden_config = config_fixture("pipeline-with-override-example.yaml")
pipeline_with_autorestart_override = config_fixture("pipeline-with-autorestart-override.yaml")
pipeline_with_automatic_deployment_creation_config = config_fixture("pipeline-with-automatic-deployment-creation.yaml")
pipeline_with_different_commit_config = config_fixture("pipeline-with-different-commit.yaml")
pipeline_with_env_vars_config = config_fixture("pipeline-edge-types-example.yaml")
pipeline_with_parameters_config = config_fixture("pipeline-with-parameters-example.yaml")
pipeline_with_retried_execution_config = config_fixture("pipeline-execution-retry-example.yaml")
pipeline_with_runtime_config_preset = config_fixture("pipeline-with-runtime-config-preset.yaml")
pipeline_with_runtime_config_preset_override_only = config_fixture("pipeline-with-override-only-preset.yaml")
pipeline_with_tasks_config = config_fixture("pipeline-with-tasks-example.yaml")
shebang_example_config = config_fixture("shebang-example.yaml")
step_with_autorestart = config_fixture("step-with-autorestart.yaml")
step_with_cache_volumes = config_fixture("step-with-cache-volumes.yaml")
step_with_partial_resources = config_fixture("step-with-partial-resources.yaml")
step_with_preset = config_fixture("step-with-preset.yaml")
step_with_resources = config_fixture("step-with-resources.yaml")
task_blueprint_pipeline_config = config_fixture("pipeline-with-task-blueprint-example.yaml")
task_config = config_fixture("task-example.yaml")
timeouts_config = config_fixture("timeouts-example.yaml")
